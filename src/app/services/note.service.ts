...
export class NoteService {
  ...
  getLowNotes(rows: TransposedRow[], noteScale: NoteScales, octave: OctaveLevels) {
    for(let i=12; i>0; i--) {  
      let semi = -1*(i + Math.abs(octave));  
      if (noteScale === NoteScales.TWELVENOTESCAL){
        var dataRow = this.transposeOneScale(semi);
        rows.push(dataRow);
      } else if (!this.isFlatSharp(semi)) {
  ...
  transposeOneScale(numOfSemiTone: number): TransposedRow {
    ...
    let toKeyCSharp = this.transposeService.transposeFixDo("1#", -12);
    console.log(`this.transposeService.transposeFixDo("1#", -12)=${toKeyCSharp}`);
